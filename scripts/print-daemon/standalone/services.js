...
export function getActiveService(now = new Date()) {
    const day = now.getDay();
    const mins = now.getHours() * 60 + now.getMinutes();
    return (SERVICES.find((s) => s.dayOfWeek === day && mins >= s.openMins && mins < s.closeMins) ?? null);
}
/** Returns the most recently-closed service today (for the "thanks, leave a
 *  review" message after Wednesday dosa night ends). */
export function getJustClosedService(now = new Date()) {
    const day = now.getDay();
    const mins = now.getHours() * 60 + now.getMinutes();
    return (SERVICES.find((s) => s.dayOfWeek === day && mins >= s.closeMins) ?? null);
}
/** Human-readable hours like "6:00 PM – 10:30 PM" */
export function formatServiceHours(s) {
    const f = (m) => {
...
export function getNextService(now = new Date()) {
    const day = now.getDay();
    const mins = now.getHours() * 60 + now.getMinutes();
    let best = SERVICES[0];
    ...
    for (const s of SERVICES) {
        ...
        if (dayDelta === 0 && mins >= s.closeMins)
            dayDelta = 7;
...
export function serviceDayName(s) {
